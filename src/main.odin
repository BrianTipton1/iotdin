...
main :: proc() {
	...
	x := protocol.serialize(
		...
		protocol.Connect_Packet {
			// will = protocol.Will{qos = .Exactly_Once}, TODO: need to still fix the will payload
			duplicate = false,
			username = "test",
			...
			properties = protocol.Connect_Properties {
				...
				authentication_method = "plain text",
				authentication_data = []byte{byte('P'), byte('A'), byte('S'), byte('S')},
			},
		},
	)
